...
export default function middleware(request: Request) {
  ...
  // No user-agent at all — guaranteed non-browser
  if (!ua) {
    return new Response('{"error":"Forbidden"}', {
      status: 403,
      headers: { 'Content-Type': 'application/json' },
  ...
  // Suspiciously short UA AND missing typical browser headers — likely a script.
  // Some legitimate embedded WebViews (e.g. Tauri, mini-programs) have short UAs
  // but still send Accept-Language or Sec-Fetch-* headers.
  if (ua.length < 10) {
