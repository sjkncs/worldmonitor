...
![World Monitor Dashboard](docs/assets/new-world-monitor.png)
...
### 🌍 CII Expansion: 54 Countries

The Country Instability Index now monitors **54 nations** (up from 22) with full data coverage across all scoring tables. New **adaptive weights** dynamically adjust CII component influence per country:

| Country Profile | Adjustment |
|---|---|
| 🔴 **War zones** (risk ≥ 40) | Conflict weight ↑ 20% → 30% |
| 🟡 **Authoritarian** (mult ≥ 2.0) | Unrest weight ↑ 20% → 28% |
| 🟢 **Democracies** (mult ≤ 0.5) | Info noise ↓ 20% → 12% |

</td>
<td width="50%">

### 📊 CII ↔ Market Cross-Validation

New service correlates CII scores with **34 country ETFs** and stock indices using **Pearson correlation**:

| Verdict | Signal |
|---|---|
| `confirmed` | CII ↑ + Market ↓ |
| `market_leading` | Market ↓ + CII stable |
| `noise` | CII ↑ + Market calm |
| `recovery` | CII ↓ + Market ↑ |
| `stable` | Both calm |
...
<details>
<summary><strong>🏗️ Architecture Improvements</strong></summary>

- **RefreshScheduler extraction** — Periodic refresh logic extracted from the monolithic `App.ts` into a dedicated module with:
  - Jittered intervals (±10%) to avoid thundering herd
  - Visibility-based throttling (4× slower when tab is hidden)
  - Unified in-flight deduplication via `runGuarded()` — prevents overlap between initial loads and periodic refreshes
  - Stale flush on tab re-focus with 150ms staggered re-triggers
- **MapContainer async DeckGL** — `DeckGLMap` is now dynamically imported with a `ready` Promise; `withDeck()` buffers callbacks during async import and replays them once loaded — zero lost callbacks
- **Deep link expansion** — URL `?country=XX` now resolves against all 54 tier-1 countries

</details>

<details>
<summary><strong>📺 YouTube Availability Detection</strong></summary>

New graceful degradation for networks where YouTube is blocked (e.g., China mainland behind GFW):
...
- LiveNewsPanel and LiveWebcamsPanel show localized `youtube_blocked` / `youtube_checking` messages
- Falls back to non-YouTube streams when blocked
...
| Area | Improvement |
|---|---|
| **RSS Proxy** | SSRF hardening via `validateFeedUrl()` — domain allowlist enforced on redirected URLs |
| **YouTube Embed** | XSS defense — `JSON.stringify()` replaces string interpolation for safe parameter injection |
| **Rate Limiting** | New Upstash Redis sliding-window limiter with `Retry-After` headers and fail-open on Redis errors |
| **Bot Detection** | Enhanced middleware checks browser headers beyond user-agent length |
| **Health Check** | New `/api/health` endpoint with Redis + external source probing (200/503) |
...
✅ Unit tests             — 12/12 pass (embed, cors, deploy-config)
✅ Vite production build  — 7.80s, 33 PWA precache entries
✅ CII data integrity     — 15/15 pass (54 countries × 5 tables)
✅ New service modules    — All imports/exports verified
✅ API endpoints          — Syntax + security features confirmed
```

New test suite `tests/verify-cii-data.mjs` validates:

- 54-country coverage across `COUNTRY_KEYWORDS`, `BASELINE_RISK`, `EVENT_MULTIPLIER`, `COUNTRY_BOUNDS`
- Adaptive weights function existence and integration
- RefreshScheduler `runGuarded` API
- All new service module exports
- API endpoint security features (SSRF, XSS, rate-limit, edge runtime)
...
- **Country Instability Index** — real-time stability scores for **54 monitored nations** using adaptive weighted multi-signal blend with per-country weight adjustments based on regime type and conflict status
- **CII ↔ Market Cross-Validation** — correlates CII scores with 34 country ETFs and stock indices using Pearson correlation to classify signals as confirmed, noise, market-leading, or recovery
...
- **Non-tier country support** — clicking countries outside the 54 tier-1 list opens a brief with available data (news, markets, infrastructure) and a "Limited coverage" badge; country names for non-tier countries resolve via `Intl.DisplayNames`
...
**54 tier-1 countries** receive continuous monitoring across five geopolitical categories:

- **Major powers & nuclear states** (9) — US, Russia, China, UK, France, India, Pakistan, North Korea, Israel
- **Active conflict zones** (13) — Ukraine, Syria, Yemen, Myanmar, Sudan, South Sudan, Somalia, DR Congo, Ethiopia, Afghanistan, Iraq, Libya, Haiti, Mali
- **Regional powers & strategic nations** (16) — Iran, Taiwan, Saudi Arabia, Turkey, Egypt, Nigeria, South Africa, Japan, South Korea, Australia, Indonesia, Thailand, Philippines, Vietnam, UAE, Qatar
- **Europe & NATO frontline** (9) — Germany, Poland, Romania, Norway, Sweden, Finland, Greece, Italy, Spain
- **Americas** (6) — Brazil, Mexico, Colombia, Venezuela, Argentina, Canada

Each receives a real-time instability score (0–100) computed from **adaptive weights** that adjust per country:

| Component                | Default | War Zones (risk ≥ 40) | Authoritarian (mult ≥ 2.0) | Democracies (mult ≤ 0.5) |
| ------------------------ | ------- | --------------------- | --------------------------- | ------------------------ |
| **Baseline risk**        | 40%     | 35%                   | 35%                         | 45%                      |
| **Unrest events**        | 20%     | 15%                   | 28%                         | 18%                      |
| **Security activity**    | 20%     | 30%                   | 22%                         | 25%                      |
| **Information velocity** | 20%     | 20%                   | 15%                         | 12%                      |

The adaptive weighting engine (`getAdaptiveWeights`) dynamically selects the appropriate profile based on each country's baseline risk and event multiplier values. This ensures war zones like Ukraine and Syria prioritize military/conflict signals, authoritarian states like North Korea and Myanmar amplify unrest events (where every protest is significant), and media-saturated democracies like the US and UK reduce information noise from high news volume.
...
| **IP rate limiting**           | Upstash Redis sliding-window rate limiter (`api/_rate-limit.js`) with configurable window/max, `Retry-After` and `X-RateLimit-*` response headers, and fail-open on Redis errors. Applied to RSS proxy and AI endpoints.                           |
| **SSRF prevention**            | RSS proxy validates redirect targets against the domain allowlist via `validateFeedUrl()`, preventing server-side request forgery through open redirects.                                                                                          |
| **XSS defense (embed)**        | YouTube embed endpoint uses `JSON.stringify()` for parameter injection instead of string interpolation, preventing script injection via crafted video IDs or origins.                                                                                |
| **Health monitoring**          | `/api/health` endpoint probes Redis connectivity and external source reachability, returning 200 (healthy) or 503 (degraded) with per-check breakdown for monitoring and alerting.                                                                  |
...
Contributions welcome! See [CONTRIBUTING.md](./.github/CONTRIBUTING.md) for detailed guidelines, including the Sebuf RPC framework workflow, how to add data sources and RSS feeds, and our AI-assisted development policy. The project also maintains a [Code of Conduct](./.github/CODE_OF_CONDUCT.md) and [Security Policy](./.github/SECURITY.md) for responsible vulnerability disclosure.
...
- [x] CII expansion to 54 countries (major powers, conflict zones, regional powers, NATO frontline, Americas)
- [x] Adaptive CII weights — war zones boost conflict, authoritarian states amplify unrest, democracies reduce info noise
- [x] CII ↔ Market cross-validation with Pearson correlation and 6-class verdict system (34 country ETFs)
- [x] RefreshScheduler module extraction with unified in-flight deduplication (`runGuarded`)
- [x] YouTube availability detection with graceful degradation for blocked networks (GFW)
- [x] `/api/health` aggregated health check endpoint (Redis + external sources, 200/503)
- [x] Upstash Redis sliding-window rate limiter with fail-open and `Retry-After` headers
- [x] RSS proxy SSRF hardening via redirect URL domain validation
- [x] YouTube embed XSS defense (JSON.stringify parameter injection)
- [x] MapContainer async DeckGL import with deferred callback queue
- [x] CII data integrity test suite (15 tests, 54-country × 5-table coverage)
...
**Elie Habib** ([@koala73](https://github.com/koala73)) — Creator and lead developer of World Monitor. Designed and built the entire platform from the ground up, including the tri-variant architecture, 60+ edge functions, Proto-first API framework, Tauri desktop app, 35+ real-time data layers, and the AI-powered intelligence pipeline. This project is a testament to extraordinary engineering vision and execution.

> 🙏 We deeply respect and appreciate the immense effort that went into building World Monitor. It is one of the most comprehensive open-source OSINT platforms available, and we are honored to contribute to it.
...
<tr>
<td align="center">
<b>v2.6.0 Contributors</b><br/>
Contributions to CII expansion, architecture optimization, security hardening, and multilingual documentation.
</td>
</tr>
</table>

#### v2.6.0 Contribution Summary

| Area | Contribution | Files |
|---|---|---|
| 🌍 **CII Expansion** | Extended Country Instability Index from 22 → 54 countries with adaptive weighting engine | `country-instability.ts`, `countries.ts` |
| 🏗️ **Architecture** | Extracted RefreshScheduler module from monolithic App.ts with unified in-flight deduplication | `refresh-scheduler.ts`, `App.ts` |
| 📊 **Cross-Validation** | New CII ↔ Market service correlating instability scores with 34 country ETFs via Pearson correlation | `cii-market-crosscheck.ts` |
| 📺 **YouTube Detection** | Graceful degradation for blocked networks (GFW) with session-cached availability probing | `youtube-availability.ts`, `LiveNewsPanel.ts`, `LiveWebcamsPanel.ts` |
| 🔒 **Security** | SSRF hardening, XSS defense, Redis rate limiter, health check endpoint, improved bot detection | `rss-proxy.js`, `embed.js`, `_rate-limit.js`, `health.js`, `middleware.ts` |
| 🗺️ **Map Optimization** | Async DeckGL import with deferred callback queue preventing lost callbacks during loading | `MapContainer.ts` |
| 🌐 **Multilingual Docs** | README translated to 7 languages (ZH/JA/KO/AR/DE/IT/FR) + Chinese CHANGELOG & SECURITY | `docs/i18n/` (10 files) |
| 🧪 **Testing** | CII data integrity test suite (15 tests, 54-country × 5-table coverage) | `verify-cii-data.mjs` |
