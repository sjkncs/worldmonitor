...
// DeckGLMap is dynamically imported to keep it out of the main bundle.
// Mobile users (SVG fallback) never download the ~155KB deck.gl chunk.
...
export class MapContainer {
  ...
  /** Resolves once the active map implementation is ready (after async DeckGLMap import). */
  public readonly ready: Promise<void>;
  /** Buffered calls to replay on deckGLMap once the async import resolves. */
  private deferredCalls: Array<(map: DeckGLMapType) => void> = [];
  ...
  private async init(): Promise<void> {
    if (this.useDeckGL) {
      ...
      try {
        ...
        this.deckReady = true;
        // Replay any calls that were buffered while the import was in-flight.
        for (const fn of this.deferredCalls) fn(this.deckGLMap);
        this.deferredCalls = [];
      } catch (error) {
  ...
  /**
   * If deck.gl is ready, execute fn immediately; otherwise buffer it.
   * No-ops silently when not in deck.gl mode (SVG/mobile).
   */
  private withDeck(fn: (map: DeckGLMapType) => void): void {
    if (!this.useDeckGL) return;
    if (this.deckReady && this.deckGLMap) {
      fn(this.deckGLMap);
    } else {
  ...
  public onHotspotClicked(callback: (hotspot: Hotspot) => void): void {
    if (this.useDeckGL) {
      this.withDeck(m => m.setOnHotspotClick(callback));
    } else {
  ...
  public onTimeRangeChanged(callback: (range: TimeRange) => void): void {
    if (this.useDeckGL) {
      this.withDeck(m => m.setOnTimeRangeChange(callback));
    } else {
  ...
  public setOnLayerChange(callback: (layer: keyof MapLayers, enabled: boolean, source: 'user' | 'programmatic') => void): void {
    if (this.useDeckGL) {
      this.withDeck(m => m.setOnLayerChange(callback));
    } else {
  ...
  public onStateChanged(callback: (state: MapContainerState) => void): void {
    if (this.useDeckGL) {
      this.withDeck(m => m.setOnStateChange((state) => {
  ...
  public onCountryClicked(callback: (country: CountryClickPayload) => void): void {
    if (this.useDeckGL) {
      this.withDeck(m => m.setOnCountryClick(callback));
    }
  }
