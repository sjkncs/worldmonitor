...
export class LiveWebcamsPanel extends Panel {
  ...
  private render(): void {
    ...
    void this.renderWithYouTubeCheck();
  }

  private async renderWithYouTubeCheck(): Promise<void> {
    // Desktop embed proxy routes through cloud bridge — skip client-side check.
    if (!isDesktopRuntime()) {
      if (getYouTubeStatus() === 'unknown') {
        this.content.innerHTML = `<div class="webcam-placeholder">&#8987; ${t('components.liveNews.checkingYouTube') ?? 'Checking YouTube availability\u2026'}</div>`;
      }
