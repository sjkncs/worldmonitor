...
import { isYouTubeAvailable, getYouTubeStatus } from '@/services/youtube-availability';
...
export class LiveNewsPanel extends Panel {
  ...
  private resumeFromIdle(): void {
    if (this.wasPlayingBeforeIdle && !this.isPlaying) {
      this.isPlaying = true;
      this.updateLiveIndicator();
      void this.checkYouTubeAndInitialize();
    }
  }
  ...
  private async checkYouTubeAndInitialize(): Promise<void> {
    // Skip check for desktop embed proxy — it routes through our cloud bridge.
    if (this.useDesktopEmbedProxy) {
      void this.initializePlayer();
      return;
    }
    ...
    // Show checking state
    if (getYouTubeStatus() === 'unknown') {
      this.showYouTubeCheckingMessage();
    }

    const available = await isYouTubeAvailable();
    if (!available) {
  ...
  private showYouTubeCheckingMessage(): void {
    this.content.innerHTML = `
      <div class="live-offline">
        <div class="offline-icon">&#8987;</div>
        <div class="offline-text">${t('components.liveNews.checkingYouTube') ?? 'Checking YouTube availability\u2026'}</div>
      </div>
    `;
  }

  private showYouTubeBlockedMessage(): void {
    this.content.innerHTML = `
      <div class="live-offline">
        <div class="offline-icon">&#128274;</div>
        <div class="offline-text">${t('components.liveNews.youtubeBlocked') ?? 'YouTube is not available on your network'}</div>
        <div class="offline-hint" style="font-size:0.8em;opacity:0.7;margin-top:4px">${t('components.liveNews.youtubeBlockedHint') ?? 'Live streams require access to YouTube. Try using a VPN or check your network settings.'}</div>
      </div>
    `;
  }
