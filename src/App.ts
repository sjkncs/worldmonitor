...
export class App {
  ...
  private handleDeepLinks(): void {
    ...
    if (url.pathname === '/story' || url.searchParams.has('c')) {
      ...
      if (countryCode) {
        ...
        const countryName = TIER1_COUNTRIES[countryCode.toUpperCase()] || countryCode;
    ...
    this.container.innerHTML = `
      <div class="header">
        ...
        : '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/></svg>'}
          ...
          <a class="agent-nav-btn" id="agentNavBtn" href="/agent.html" title="AI Agent">🤖 Agent</a>
  ...
  private createPanels(): void {
    ...
    // Defer early map calls until async DeckGLMap import resolves (no-ops on SVG/mobile).
    const mapRef = this.map;
    mapRef.ready.then(() => {
      mapRef.initEscalationGetters();
      this.currentTimeRange = mapRef.getTimeRange();
    }).catch(() => {});
  ...
  private setupEventListeners(): void {
    ...
    this.boundVisibilityHandler = () => {
      ...
      if (document.hidden) {
        this.scheduler.hiddenSince = this.scheduler.hiddenSince || Date.now();
        mlWorker.unloadOptionalModels();
      } else {
        this.resetIdleTimer();
        this.scheduler.flushStale();
      }
    };
  ...
  private async loadAllData(): Promise<void> {
    const g = (name: string, fn: () => Promise<void>) => this.scheduler.runGuarded(name, fn);

    const tasks: Array<{ name: string; task: Promise<void> }> = [
      { name: 'news', task: g('news', () => this.loadNews()) },
      { name: 'markets', task: g('markets', () => this.loadMarkets()) },
      { name: 'predictions', task: g('predictions', () => this.loadPredictions()) },
      { name: 'pizzint', task: g('pizzint', () => this.loadPizzInt()) },
      { name: 'fred', task: g('fred', () => this.loadFredData()) },
      { name: 'oil', task: g('oil', () => this.loadOilAnalytics()) },
      { name: 'spending', task: g('spending', () => this.loadGovernmentSpending()) },
    ];
    ...
    if (SITE_VARIANT === 'full') {
      tasks.push({ name: 'intelligence', task: g('intelligence', () => this.loadIntelligenceSignals()) });
    }
    ...
    if (SITE_VARIANT === 'full') tasks.push({ name: 'firms', task: g('firms', () => this.loadFirmsData()) });
    if (this.mapLayers.natural) tasks.push({ name: 'natural', task: g('natural', () => this.loadNatural()) });
    if (this.mapLayers.weather) tasks.push({ name: 'weather', task: g('weather', () => this.loadWeatherAlerts()) });
    if (this.mapLayers.ais) tasks.push({ name: 'ais', task: g('ais', () => this.loadAisSignals()) });
    if (this.mapLayers.cables) tasks.push({ name: 'cables', task: g('cables', () => this.loadCableActivity()) });
    if (this.mapLayers.cables) tasks.push({ name: 'cableHealth', task: g('cableHealth', () => this.loadCableHealth()) });
    if (this.mapLayers.flights) tasks.push({ name: 'flights', task: g('flights', () => this.loadFlightDelays()) });
    if (CYBER_LAYER_ENABLED && this.mapLayers.cyberThreats) tasks.push({ name: 'cyberThreats', task: g('cyberThreats', () => this.loadCyberThreats()) });
    if (this.mapLayers.techEvents || SITE_VARIANT === 'tech') tasks.push({ name: 'techEvents', task: g('techEvents', () => this.loadTechEvents()) });

    // Tech Readiness panel (tech variant only)
    if (SITE_VARIANT === 'tech') {
      tasks.push({ name: 'techReadiness', task: g('techReadiness', () => (this.panels['tech-readiness'] as TechReadinessPanel)?.refresh()) });
    }
  ...
  private async loadDataForLayer(layer: keyof MapLayers): Promise<void> {
    if (this.scheduler.isRunning(layer)) return;
    this.scheduler.markInFlight(layer);
    ...
    try {
      ...
      this.scheduler.clearInFlight(layer);
  ...
  private scheduleRefresh(
    ...
    this.scheduler.schedule({ name, fn, intervalMs, condition });
  }
