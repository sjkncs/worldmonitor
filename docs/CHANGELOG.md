...
**🌍 CII Expansion to 54 Countries + Adaptive Weights** — The Country Instability Index now monitors 54 nations (up from 22), covering all major powers, active conflict zones, regional powers, NATO frontline states, and key Americas nations. A new adaptive weighting engine dynamically adjusts CII component weights based on each country's characteristics — war zones boost conflict weight, authoritarian states amplify unrest signals, and media-saturated democracies reduce information noise.

**🏗️ Architecture: RefreshScheduler Extraction** — The monolithic `App.ts` refresh logic has been extracted into a dedicated `RefreshScheduler` module with jittered intervals, visibility-based throttling, conditional execution, and unified in-flight deduplication via `runGuarded()`. This eliminates the standalone `inFlight` Set and prevents overlap between initial data loads and periodic refreshes.

**📊 CII ↔ Market Cross-Validation** — A new service correlates CII scores with country ETF and stock index performance using Pearson correlation analysis. Per-country verdicts classify signals as `confirmed` (CII rising + market falling), `noise` (CII rising but market calm), `market_leading` (market falling but CII stable), `recovery`, or `stable`.

### Added

- **CII country expansion (22 → 54)** — expanded `TIER1_COUNTRIES`, `COUNTRY_KEYWORDS`, `BASELINE_RISK`, `EVENT_MULTIPLIER`, `ISO3_TO_ISO2`, `COUNTRY_BOUNDS`, `ZONE_COUNTRY_MAP`, and `HOTSPOT_COUNTRY_MAP` to cover 54 countries across 5 geopolitical categories
- **Adaptive CII weights** — `getAdaptiveWeights()` dynamically adjusts component weights per country: war zones (baseline risk ≥40) boost conflict from 20% → 30%, authoritarian states (event multiplier ≥2.0) boost unrest from 20% → 28%, media-saturated democracies (multiplier ≤0.5) reduce information weight from 20% → 12%
- **CII ↔ Market cross-validation service** (`cii-market-crosscheck.ts`) — correlates CII scores with 34 country ETFs (iShares, Vanguard) and local stock indices via `MarketServiceClient`, with Pearson correlation, divergence scoring, and 6-class verdict system
- **RefreshScheduler module** (`refresh-scheduler.ts`) — extracted from `App.ts` with `schedule()`, `runGuarded()`, `flushStale()`, `markInFlight()`/`clearInFlight()`, `isRunning()`, and `destroy()` APIs
- **YouTube availability detection** (`youtube-availability.ts`) — probes `youtube.com/favicon.ico` with `no-cors` mode, 5-second timeout, session-lifetime caching, and automatic re-probe on `online` events. LiveNewsPanel and LiveWebcamsPanel show localized "YouTube blocked" / "Checking YouTube..." messages when unavailable
- **Health check endpoint** (`/api/health`) — aggregated probe checking Upstash Redis connectivity and external source reachability (GitHub API, BBC RSS, YouTube) with 200/503 status, elapsed timing, and per-check breakdown
- **Rate limiting utility** (`api/_rate-limit.js`) — Upstash Redis sliding-window counters with configurable window/max, IP identification via `x-forwarded-for`, pipeline INCR+EXPIRE, proper `Retry-After` and `X-RateLimit-*` headers, and fail-open on Redis errors
- **CII data integrity tests** (`tests/verify-cii-data.mjs`) — 15 automated tests verifying 54-country coverage across all CII data tables, adaptive weights integration, scheduler exports, API endpoint security features

### Security

- **RSS proxy SSRF hardening** — `validateFeedUrl()` prevents server-side request forgery via domain allowlist enforcement on redirected URLs
- **YouTube embed XSS defense** — replaced string interpolation with `JSON.stringify()` for safe parameter injection into generated HTML
- **Middleware bot detection** — improved crawler identification with browser header checks in addition to user-agent length validation
- **Rate limiting on RSS proxy** — integrated `checkRateLimit()` to prevent API abuse

### Changed

- **App.ts scheduler integration** — replaced standalone `inFlight: Set<string>` with `RefreshScheduler` instance; `loadAllData()` and `loadDataForLayer()` now use `scheduler.runGuarded()` / `scheduler.markInFlight()` / `scheduler.clearInFlight()`
- **MapContainer async initialization** — DeckGLMap is now dynamically imported with a `ready` promise; `withDeck()` buffers callbacks in a deferred queue and replays them once the import resolves, preventing lost callbacks during async loading
- **Deep link country support** — URL `?country=XX` parameter now resolves against all 54 `TIER1_COUNTRIES` instead of 22
- **i18n additions** — `youtube_blocked` and `youtube_checking` keys added to `en.json` for YouTube availability UI states

### Fixed

- **Missing COUNTRY_BOUNDS for UAE and Qatar** — added bounding boxes `AE: [22, 26, 51, 56]` and `QA: [24, 27, 50, 52]` that were missing from the 54-country expansion
